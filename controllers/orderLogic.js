...
exports.LimitBuyOrderList = async function (type, user_id, amount, withFeeAmount, isMarket, price, pair_id, fee, left_coin_id, right_coin_id, res) {
    ...
    while (currentAmount > 0) {
        ...
        if (result.length > 0) {
            ...
            var remainingAmount = result[0].remaining_amount; // 20 slc for buy
            ...
            if (parseFloat(remainingAmount) > parseFloat(currentAmount)) {
                ...
                executeAmountForFind = currentAmount

                remainingAmount = remainingAmount - currentAmount;
                currentAmount = 0;

            } else if (parseFloat(remainingAmount) < parseFloat(currentAmount)) {
                ...
                executeAmountForCreator = remainingAmount; // by KG
            ...
            if (type == 'BUY') {
                ...
                await promisePool.query(`UPDATE user_wallet SET balanceInOrder=balanceInOrder-${executeAmountForFind} WHERE coin_id = ${left_coin_id} AND user_id=${Find_user_id}`);
                ...
                console.log('7777', executeAmountForFind, old_price, `UPDATE user_wallet SET balanceInOrder=balanceInOrder-${executeAmountForFind} WHERE coin_id = ${left_coin_id} AND user_id=${Find_user_id}`)
                console.log('10000', executeAmountForCreator, price, `UPDATE user_wallet SET  balanceInOrder=balanceInOrder-${parseFloat(executeAmountForCreator * price)} WHERE coin_id = ${right_coin_id} AND user_id=${user_id}`)
                console.log('11111', executeAmountForFind, old_price, `UPDATE user_wallet SET Balance=Balance+${parseFloat(executeAmountForCreator * old_price).toFixed(5)} WHERE coin_id = ${right_coin_id} AND user_id=${Find_user_id}`)
                console.log('88888', executeAmountForCreator, price, `UPDATE user_wallet SET Balance=Balance+${parseFloat(executeAmountForCreator).toFixed(5)} WHERE coin_id = ${left_coin_id} AND user_id=${user_id}`)
                ...
                // let oldUserLeftCoinAmount =  (((parseFloat(executeAmountForFind) * parseFloat(price)).toFixed(5)) / parseFloat(old_price)).toFixed(5);

                await promisePool.query(`UPDATE user_wallet SET  balanceInOrder=balanceInOrder-${parseFloat(parseFloat(executeAmountForFind * old_price)).toFixed(5)} WHERE coin_id = ${right_coin_id} AND user_id=${Find_user_id} `);
                await promisePool.query(`UPDATE user_wallet SET balanceInOrder=balanceInOrder-${parseFloat(executeAmountForFind).toFixed(5)} WHERE coin_id = ${left_coin_id} AND user_id=${user_id}`);
                await promisePool.query(`UPDATE user_wallet SET Balance=Balance+${parseFloat(executeAmountForFind).toFixed(5)} WHERE coin_id = ${left_coin_id} AND user_id=${Find_user_id}`);
                await promisePool.query(`UPDATE user_wallet SET Balance=Balance+${parseFloat(executeAmountForCreator*price).toFixed(5)} WHERE coin_id = ${right_coin_id} AND user_id=${user_id}`);


                console.log('finduserbalanceinordre', executeAmountForFind, old_price, `UPDATE user_wallet SET  balanceInOrder=balanceInOrder-${parseFloat(parseFloat(executeAmountForFind * old_price)).toFixed(5)} WHERE coin_id = ${right_coin_id} AND user_id=${Find_user_id}`)
                console.log('newuserinorder', executeAmountForFind, price, `UPDATE user_wallet SET balanceInOrder=balanceInOrder-${parseFloat(executeAmountForFind).toFixed(5)} WHERE coin_id = ${left_coin_id} AND user_id=${user_id}`)
                console.log('c', executeAmountForFind, old_price, `UPDATE user_wallet SET Balance=Balance+${parseFloat(executeAmountForFind).toFixed(5)} WHERE coin_id = ${left_coin_id} AND user_id=${Find_user_id}`)
                console.log('d', executeAmountForCreator, price, `UPDATE user_wallet SET Balance=Balance+${parseFloat(executeAmountForCreator*price).toFixed(5)} WHERE coin_id = ${right_coin_id} AND user_id=${user_id}`)
